window.onload = function() {
  ...
  function setURI(message, font_size=undefined, font_family=undefined, colors=window.default_color_sequence){
    if(message === ''){
      ...
      if(colors != window.default_color_sequence){
        color_sequence_for_uri = encodeURIComponent(`<wbr>&color_sequence=${colors}`)
      }

      document.querySelector("[data-id='output-html']").innerHTML = document.location +
        `?<wbr>font-size=${font_size}` + color_sequence_for_uri + `<wbr>&font-family=${font_family.replace(/ /g, '%20')}` + `<wbr>&message=${encodeURIComponent(message)}`;
    }
  }
  ...
  // make page display text
    ...
    else
    {
      ...
      var message = '';
      try{
        message = decodeURIComponent(document.location.search.match(/message=(.*)/)[1]);
      }catch(e){
