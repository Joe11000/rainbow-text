...
(function () {
  ...
  function createColorSelectors(colors_str) {
    ...
    colors_str.split(',').forEach(function (color) {
  ...
  function setURI(args) {
    var message = args['message'] || getText();
    var font_size = args['font_size'] || getFontSize();
    var font_family = args['font_family'] || getFontFamily();
    var colors = args['colors'] || getColors();
    debugger;
    ...
    var color_sequence_for_uri = '';
    if (colors != window.default_color_sequence) {
      color_sequence_for_uri = '&color_sequence=' + encodeURIComponent(colors);
    }

    var message_sequence_for_uri = '&message=' + encodeURIComponent(encodeURIComponent(message));
    document.querySelector("[data-id='output-html']").innerHTML = document.location.origin + document.location.pathname + ('?font-size=' + font_size) + color_sequence_for_uri + ('&font-family=' + font_family.replace(/ /g, '%20')) + message_sequence_for_uri;
  }
